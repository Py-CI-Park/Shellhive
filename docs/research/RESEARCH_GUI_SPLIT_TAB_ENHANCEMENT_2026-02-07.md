...
Shellhive는 여러 AI CLI 프로젝트를 동시에 실행하고 관리하는 GUI 데스크톱 애플리케이션입니다. 현재까지 Phase 1~5가 완료되어 기본적인 탭 관리, 프로젝트 관리, 스니펫, 설정, 키보드 단축키 기능이 구현되어 있습니다.
...
```
사용자: "프로젝트 A에서 Claude Code와 Gemini CLI를 동시에 실행하며 비교하고 싶다"

동작:
1. 프로젝트 A 클릭 → 첫 번째 터미널 생성
2. Ctrl+Shift+D → 화면 수평 분할
3. 새 분할 영역에 자동으로 같은 프로젝트 경로로 터미널 생성
4. 왼쪽에서 "claude" 실행, 오른쪽에서 "gemini" 실행
5. Alt+화살표로 패널 간 빠른 전환
```
...
4. **AI CLI 특화**: 병렬 AI 에이전트 워크플로우 지원
